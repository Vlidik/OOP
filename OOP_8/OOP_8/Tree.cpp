...
std::shared_ptr<TreeItem<T>> Tree<T>::deleteNode(std::shared_ptr<TreeItem<T>> root, int32_t side)
{
	...
	if (side < root->item->Side()) {
		...
		if (root->left == nullptr) {
			...
			std::shared_ptr<TreeItem<T>> parent = root->parent;
			temp->parent = parent;
			root->left = nullptr;
			root->right = nullptr;
			root->parent = nullptr;
...
TIterator<TreeItem<T>, T> Tree<T>::begin()
{
	...
	return TIterator<TreeItem<T>, T>(tmp);
}
...
template <class T>
void Tree<T>::Balanced(std::shared_ptr<T> *&arr, int l, int r)
{
	if (l > r) {
		return;
	}
...
template <class T>
void Tree<T>::recClear(std::shared_ptr<TreeItem<T>> node)
{
	if (!node) {
		return;
	}
